...
var fnProcesarDescarga = function (descargaId, callback) {
    fnGetDescargaDetalle(descargaId, function (err, res) {
        ...
        var numReg = 0;
        var totalReg = lecturas.length;
        async.eachSeries(lecturas,
            function (lectura, callback2) {
                ioAPI.sendProgress("Procesando lecturas...", ++numReg, totalReg);
...
var fnActualizarUltimoControl = function (callback) {
    async.series([
        function (callback2) {
            ioAPI.sendProgress("Actualizar último control...", 1, 4);
            var connection = getConnection();
        ...
        function (callback2) {
            ioAPI.sendProgress("Actualizar último control...", 2, 4);
            var connection = getConnection();
        ...
        function (callback2) {
            ioAPI.sendProgress("Actualizar último control...", 3, 4);
            var connection = getConnection();
        ...
        function (err, results) {
            ...
            // todo bien
            ioAPI.sendProgress("Actualizar último control...", 4, 4);
            callback(null, null);
        });
    ...
    async.series({
        ...
        processPuntoLeido: function (callback2) {
            rondaRealizadaDb.getPuntoRondaRealizadaCarga(rondaRealizadaId, lectura.tipoId, function (err, res) {
                ...
                var puntos = res;
                // elimiar es solo para test
                // if (lectura.tag == '0424681958') {
                //     var test = 0;
                // }
                if (puntos.length > 0 && !puntos[0].fecha) {
                    // ha encontrado el punto
