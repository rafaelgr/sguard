...
module.exports.getCorregirDescarga = function (descargaId, callback) {
    var connection = getConnection();
    var sql = ""
    async.waterfall(
        [
    ...
    function fnPutDescarga(descargaId, nterminal, procesada, callback) {
        var connection = getConnection();
        sql = "UPDATE descargas SET nterminal = ?, procesada = ? WHERE descargaId = ?";
        sql = mysql.format(sql, [nterminal, procesada, descargaId]);
        connection.query(sql, function (err, result) {
            closeConnectionCallback(connection, callback);
            if (err) {
                callback(err);
    ...
    module.exports.putDescarga = fnPutDescarga;
