...
module.exports.comprobarTiempoEntreRondas = function(rondaId, fecha, callback) {
    ...
    // Solamente comprobamos cuando no sea la primera en el dia
    rondaRealizadaDb.getPrimeraVezRondaEnDia(rondaId, fecha, function(err, res){
        if (err){
            logMens += "ERROR: " + err.Message + "<br/>";
            return callback(err);
        }
        if (res && ronda.lastcontrol){
            // es la primera vez, no se controla nada
            // y la ronda ya ha sido conttrolada una vez
            return callback(null, null);
        }
        var connection = getConnection();
        var rondas = null;
