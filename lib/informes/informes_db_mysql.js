...
function fromDbToJsRondas(dfecha, hfecha, regs) {
    ...
    for (var i = 0; i < regs.length; i++) {
    	...
    	rfecha = moment(new Date(reg.fecha)).format("YYYY-MM-DD");
    	var punto = {
    		ordenleido: reg.ordenleido,
    		ordenronda: reg.ordenronda,
    		tagleido: reg.tagleido,
    		punto: reg.punto,
    		fecha: moment(rfecha).format("DD/MM/YYYY"),
    		hora: reg.hora,
...
module.exports.getRondas = function(dfecha, hfecha, callback) {
    ...
    var grupos = null;
    var sql = "SELECT rr.rondaRealizadaId, rrp.rondaRealizadaPuntoId,"
    sql += " r.nombre AS nronda, v.nombre AS vigilante, rr.fecha AS rfecha, rr.hora AS rhora, rr.resultado AS rresultado,";
    sql += " rrp.ordenleido, rrp.orden AS ordenronda, rrp.tagleido, rrp.nombre AS punto, rrp.fecha, rrp.hora, rrp.resultado";
