...
// a partir de los registros pasados en regs
// monta un objeto con el formato necesario para devolver un informe
function fromDbToJsRondasTerminal(dfecha, hfecha, regs) {
    var inf = {
...
// a partir de los registros pasados en regs
// monta un objeto con el formato necesario para devolver un informe
function fromDbToJsRondasPunto(dfecha, hfecha, regs) {
    var inf = {
...
// getRondasTerminal
// Lee las rondas realizadas entre las fechas indicadas
module.exports.getRondasTerminal = function(terminalId, dfecha, hfecha, callback) {
    var connection = getConnection();
    var grupos = null;
    ...
    connection.query(sql, function(err, result) {
        ...
        grupos = result;
        callback(null, fromDbToJsRondasTerminal(dfecha, hfecha, result));
    });
    closeConnectionCallback(connection, callback);
}
...
// getRondasPunto
// Lee las rondas realizadas entre las fechas indicadas
module.exports.getRondasPunto = function(puntoId, dfecha, hfecha, callback) {
    var connection = getConnection();
    var grupos = null;
    ...
    sql += " LEFT JOIN vigilantes AS v ON v.vigilanteId = rr.vigilanteId";
    sql += " LEFT JOIN terminales AS t ON t.terminalId = rr.terminalId";
    sql += " WHERE p.puntoId = ? AND rr.fecha >= ? AND rr.fecha <= ?"
    ...
    connection.query(sql, function(err, result) {
        ...
        grupos = result;
        callback(null, fromDbToJsRondasPunto(dfecha, hfecha, result));
    });
    closeConnectionCallback(connection, callback);
}
