/*-------------------------------------------
Controlador encargado de los informes
--------------------------------------------*/
var express = require('express');
var router = express.Router();
var informesDb = require("./informes_db_mysql");

router.get('/rondas', function(req, res) {
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    informesDb.getRondas(dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas2', function(req, res) {
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    informesDb.getRondas2(dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/ano', function(req, res) {
    var ano = req.query.ano;
    if (!ano) {
        // si no es así devolvemos un error indicándolo
        res.status(400).send("Parámetros incorrectos");
        return;
    }
    informesDb.getComparativoRondasAnual(ano, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/general', function(req, res) {
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var ronda = req.query.ronda;
    var vigilante = req.query.vigilante;
    var terminal = req.query.terminal;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    if (!ronda) {
        ronda = "*";
    }
    if (!vigilante) {
        vigilante = "*";
    }
    if (!terminal) {
        terminal = "*";
    }
    informesDb.getGeneral(ronda, vigilante, terminal, dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/general2', function(req, res) {
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var ronda = req.query.ronda;
    var vigilante = req.query.vigilante;
    var terminal = req.query.terminal;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    if (!ronda) {
        ronda = "*";
    }
    if (!vigilante) {
        vigilante = "*";
    }
    if (!terminal) {
        terminal = "*";
    }
    informesDb.getGeneral2(ronda, vigilante, terminal, dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/generalexcel', function(req, res) {
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var ronda = req.query.ronda;
    var vigilante = req.query.vigilante;
    var terminal = req.query.terminal;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    if (!ronda) {
        ronda = "*";
    }
    if (!vigilante) {
        vigilante = "*";
    }
    if (!terminal) {
        terminal = "*";
    }
    informesDb.getGeneralExcel(ronda, vigilante, terminal, dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/vigilante', function(req, res) {
    var vigilanteId = req.query.vigilanteId;
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha || !vigilanteId) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    informesDb.getRondasVigilante(vigilanteId, dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/vigilante2', function(req, res) {
    var vigilanteId = req.query.vigilanteId;
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha || !vigilanteId) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    informesDb.getRondasVigilante2(vigilanteId, dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/terminal', function(req, res) {
    var terminalId = req.query.terminalId;
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }    
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha || !terminalId) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    informesDb.getRondasTerminal(terminalId, dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/terminal2', function(req, res) {
    var terminalId = req.query.terminalId;
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }    
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha || !terminalId) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    informesDb.getRondasTerminal2(terminalId, dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});


router.get('/rondas/punto', function(req, res) {
    var puntoId = req.query.puntoId;
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }    
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha || !puntoId) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    informesDb.getRondasPunto(puntoId, dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/punto2', function(req, res) {
    var puntoId = req.query.puntoId;
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }    
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha || !puntoId) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    informesDb.getRondasPunto2(puntoId, dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/punto3', function(req, res) {
    var puntos = req.query.puntos;
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }    
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha || !puntos) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    informesDb.getRondasPunto3(puntos, dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/punto4', function(req, res) {
    var puntos = req.query.puntos;
    var dFecha = req.query.dfecha;
    var hFecha = req.query.hfecha;
    var dHora = req.query.dhora;
    var hHora = req.query.hhora;
    if (!dHora) {
        dHora = "*";
    }
    if (!hHora) {
        hHora = "*";
    }    
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!dFecha || !hFecha || !puntos) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    informesDb.getRondasPunto4(puntos, dFecha, hFecha, dHora, hHora, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

router.get('/rondas/plantilla', function(req, res) {
    var rondaId = req.query.rondaId;
    // comprobamos que nos han pasado los parámetros que corresponden
    if (!rondaId) {
        // si no es así devolvemos un error indicándolo
        res.status(401).send("Parámetros incorrectos");
        return;
    }
    informesDb.getRondasPlantilla(rondaId, function(err, reg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(reg);
        }
    });
});

// Exports
module.exports = router;
