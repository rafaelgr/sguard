...
// getPrimeraVezRonda
// controla si la ronda ya ha sido realizada alguna vez
// si es la primera vez devuelve cierto, si no falso.
module.exports.getPrimeraVezRonda = function (id, callback) {
    var connection = getConnection();
    var veces = 0;
    sql = "SELECT COUNT(*) AS numero FROM rondas_realizadas WHERE rondaId = ?";
    sql = mysql.format(sql, id);
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) {
        ...
        veces = result[0].numero;
        callback(null, (veces == 0));
    });

}

// getPrimeraVezRondaEnDia
// controla si la ronda ya ha sido realizada alguna vez
// si es la primera vez devuelve cierto, si no falso.
module.exports.getPrimeraVezRondaEnDia = function (id, fecha, callback) {
    // ojo -- la fecha viene con hora formatear
    fecha = moment(fecha).format('YYYY-MM-DD');
    var connection = getConnection();
    var veces = 0;
    sql = "SELECT COUNT(*) AS numero FROM rondas_realizadas WHERE rondaId = ?";
    sql += " AND fecha = ?";
    sql = mysql.format(sql, [id, fecha]);
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) {
            callback(err, null);
        ...
        veces = result[0].numero;
        // comparamos con uno porque la que estamos testeando ya
        // está dad de alta
        callback(null, (veces == 1));
    });

}

// getPrimeraVezRondaTag
// controla si la ronda ya ha sido realizada alguna vez
// si es la primera vez devuelve cierto, si no falso.
module.exports.getPrimeraVezRondaTag = function (tag, callback) {
    var connection = getConnection();
    var veces = 0;
    sql = "SELECT COUNT(*) AS numero FROM rondas_realizadas";
    sql += " WHERE rondaId = (";
    sql += " SELECT rondaId FROM rondas WHERE tag = ?);";
...
// getPrimeraVezRondaEnDiaTag
// controla si la ronda ya ha sido realizada alguna vez
// si es la primera vez devuelve cierto, si no falso.
module.exports.getPrimeraVezRondaEnDiaTag = function (tag, fecha, callback) {
    var connection = getConnection();
    var veces = 0;
    sql = "SELECT COUNT(*) AS numero FROM rondas_realizadas";
    sql += " WHERE rondaId = (";
    sql += " SELECT rondaId FROM rondas WHERE tag = ?)";
    sql += " AND fecha = ?";
    sql = mysql.format(sql, [tag, fecha]);
    connection.query(sql, function (err, result) {
        connection.end();
        if (err) {
            callback(err, null);
...
module.exports.borrarRondasDeDescarga = function (descargaId, done) {
    async.series(
        [
            function borraLineas(done2) {
                var con = getConnection();
                var sql = "DELETE FROM rondas_realizadaspuntos WHERE rondaRealizadaId  IN";
                sql += " (SELECT rondaRealizadaId FROM rondas_realizadas WHERE descargaId = ?)";
            ...
            function borraCabecera(done2) {
                var con = getConnection();
                var sql = "DELETE FROM rondas_realizadas WHERE descargaId = ?";
                sql = mysql.format(sql, descargaId );
                con.query(sql, function(err, res){
                    closeConnection(con);
                    if (err) return done2(err);
        ...
        fnPostRondaRealizada(rondaRealizada, function (err, result) {
            ...
            async.eachSeries(ronda.puntos,
                ...
                function (err) {
                    ...
                    return callbackR(null, rondaRealizada, mintime, maxtime);
                });
        });
    })
        ...
        fnRondaRealizadaConPuntosFaltantes(id, function (err, res) {
            ...
            fnRondaRealizadaConPuntosExcedidos(id, function (err, res) {
                ...
                if (res.length > 0) {
                    // hay puntos faltantes
                    if (rondaRealizada.resultado == "CORRECTO") {
                        rondaRealizada.resultado = "SUPERADO TIEMPO EN PUNTOS ENTRE RONDAS";
                        rondaRealizada.validada = 0;
                        rondaRealizada.obsvalida = "";
                    } else {
                        rondaRealizada.resultado += " / SUPERADO TIEMPO EN PUNTOS ENTRE RONDAS";
                ...
                var connection = getConnection();
                // eliminamos los puntos para evitar errores de sinyaxis SQL en el UPDATE
                delete rondaRealizada.puntos;
                sql = "UPDATE rondas_realizadas SET ? WHERE rondaRealizadaId = ?";
...
module.exports.putPuntoRondaRealizada = function (id, puntoRondaRealizada, callback) {
    var connection = getConnection();
    sql = "UPDATE rondas_realizadaspuntos SET ? WHERE rondaRealizadaPuntoId = ?";
    sql = mysql.format(sql, [puntoRondaRealizada, id]);
    connection.query(sql, function (err, result) {
