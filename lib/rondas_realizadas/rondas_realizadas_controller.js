...
router.put('/puntorondarealizada/:rondaRealizadaPuntoId', function (req, res) {
    rondasDb.putPuntoRondaRealizada(req.params.rondaRealizadaPuntoId, req.body.puntoRondaRealizada, function (err, rondaRealizada) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(rondaRealizada);
...
router.delete('/descargas/:descargaId', function (req, res) {
    var descargaId = req.params.descargaId;
    rondasDb.borrarRondasDeDescarga(descargaId, function (err, result) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(null);
