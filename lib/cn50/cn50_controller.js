...
// GetDescargasLineas
// Obtiene las lineas de descarga de un fichero sdf
router.get('/descargas-lineas', function(req, res) {
    var file = req.query.file;
    var terminalId = req.query.terminalId;
    cn50Db.getDescargasLineas(file, terminalId, function(err, msg) {
        if (err) {
            res.status(500).send(err.message);
        } else {
            res.json(msg);
