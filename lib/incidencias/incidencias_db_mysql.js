...
module.exports.getIncidenciasBuscar = function(nombre, callback) {
    var connection = getConnection();
    var incidencias = null;
    var sql = "SELECT * FROM incidencias";
    if (nombre !== "*") {
...
module.exports.postIncidencia = function(incidencia, callback) {
    if (!comprobarIncidencia(incidencia)) {
        var err = new Error("La incidencia pasada es incorrecta, no es un objeto de este tipo o le falta algún atributo olbligatorio");
        callback(err);
        return;
    }
    var connection = getConnection();
